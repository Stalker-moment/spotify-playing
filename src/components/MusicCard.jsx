...
const MusicCard = ({ data }) => {
  ...
  useEffect(() => {
    ...
    const [currentMin, currentSec] = data.progress.split(':').map(Number);
    const currentTime = (currentMin * 60) + currentSec;
    setCurrentTime(currentTime);

    const [totalMin, totalSec] = data.duration.split(':').map(Number);
  ...
  return (
    <div className="max-w-sm rounded-lg overflow-hidden shadow-lg bg-white dark:bg-gray-800">
      <img className="w-full" src={data.songThumbnail} alt={data.title} />
      <div className="px-6 py-4">
        <div className="text-gray-700 dark:text-white font-bold text-xl mb-2">{data.title}</div>
        <a href={data.artistUrl} className="text-gray-700 dark:text-white text-base">{data.artist}</a>
        <p className="text-gray-700 dark:text-white text-base">{data.album}</p>
        ...
        <div className="flex items-center">
          ...
          <span className="mt-2 text-gray-700 dark:text-white ml-1"> / {data.duration}</span>
        </div>
      </div>
